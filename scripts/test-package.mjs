...
test('packaged CLI reduces and verifies after the tool is removed', async () => {
  ...
  try {
    ...
    assert.equal(metadata.dependencies.stalegreen, undefined);
    assert(!entry.files.some(file => file.path.startsWith('scripts/verification/') || file.path === 'scripts/dev-verification.mjs'));
    assert(!(await readdir(path.join(prefix, 'node_modules'))).includes('stalegreen'));
